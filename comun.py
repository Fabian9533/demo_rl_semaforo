...
def capacidad_enlace(net_file, edge_id, largo_vehiculo=5.0, min_gap=2.5):
    """Vehiculos detenidos que caben en un enlace (todos sus carriles), con 5 m de auto
    y 2.5 m de separacion: es la referencia para decir que la cola lo llena (spillback)."""
    net = sumolib.net.readNet(net_file)
    e = net.getEdge(edge_id)
    return int(e.getLength() // (largo_vehiculo + min_gap)) * e.getLaneNumber()
