...
\noindent\textbf{Aporte específico (U4: Coordinación distribuida).} Esta tesis implementa un módulo de \textbf{coordinación distribuida entre semáforos} para pasar de optimizaciones locales a \textbf{optimización de corredor}. Lo motiva un resultado propio: los agentes tabulares independientes de los escenarios de varios semáforos pierden frente a un programa fijo coordinado (Capítulo~\ref{cap:resultados}). Sus componentes son:
\begin{itemize}
    \item \textit{Estado extendido}: cada agente observa su entorno local (colas y vehículos en movimiento por aproximación, fase activa, tiempo en fase) y un \textbf{resumen de cada vecino} (fase vecina, tiempo en ella, vehículos que descargan hacia el enlace compartido y colas en ese enlace).
    \item \textit{Mensajería ligera}: canal de comunicación simulado, con latencia y pérdida de mensajes configurables, para intercambiar mensajes agent-to-agent (\emph{fase}, \emph{descarga hacia el vecino}, \emph{colas en salidas}).
    \item \textit{Recompensa con coordinación}: término explícito de \textbf{spillback} para evitar que una cola bloquee a la intersección vecina. La mezcla con una señal global (\(R_{global}\)) queda formulada y no se ejecuta: el término dirigido al vecino ya cumple esa función.
